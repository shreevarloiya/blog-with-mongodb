...
app.get("/", function(request, response) {
  ...
  Blog.find(function(err,myitems){
    if (err) {
      console.log(err);
    } else {
  ...
  Blog.find(function(err,myitems){
    if (err) {
      ...
      myitems.forEach(function(myitem) {
        if (_.lowerCase(req.params.newpath) === _.lowerCase(myitem.blogtitle)) {
        res.render("post", {newposttitle: myitem.blogtitle, newpostbody: myitem.blogarticle});
        }
      });
    }
  });
